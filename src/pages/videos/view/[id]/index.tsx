...
import { deleteSavedVideoById } from 'apiSdk/saved-videos';
...
function VideoViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<VideoInterface>(
    ...
    () =>
      getVideoById(id, {
        relations: ['content_creator', 'saved_video'],
      }),
  );

  const saved_videoHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              {hasAccess('content_creator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/content-creators/view/${data?.content_creator?.id}`}>
                      {data?.content_creator?.id}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('saved_video', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/saved-videos/create?video_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.saved_video?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/saved-videos/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('saved_video', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/saved-videos/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('saved_video', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    saved_videoHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
